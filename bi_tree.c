...
void rev_trav(struct user_states *root, struct user_states* dst, int* size)
{
   if(root == NULL)
        return ;

    rev_trav(root -> lchild, dst, size);
    copy_node(&dst[(*size) ++], root);
    rev_trav(root -> rchild, dst, size);
}

int trav_tree(struct user_states *root, struct user_states* dst)
{
    int size = 0;
